                            ...
                            <div class="col-md-6">
                                ...
                                <div class="card card" style="background-color: #f4f6f9">
                                    <div class="card-header">
                                        <h3 class="card-title" style="margin: -10px;"><b>  @lang('messages.Update Bank') </b></h3>
                                    </div>
                                    <!-- /.card-header -->
                                    <!-- form start -->
                                    <form method="post" action="{{route('account/update',$value->id)}}">
                                        @csrf
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="radio" name="type" value="cash"
                                                                   {{old('type')=="cash" ? 'checked='.'"checked"' : '' }}  @if(isset($value)) {{ ($value->type=="cash")? "checked" : "" }} @endif checked="checked">@lang('messages.Cash')
                                                            <input type="radio" name="type" value="female"
                                                                {{old('type')=="bank" ? 'checked='.'"checked"' : '' }}  @if(isset($value)) {{ ($value->type=="bank")? "checked" : "" }} @endif>@lang('messages.Bank')
